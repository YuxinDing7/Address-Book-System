...
#include "ui_relativeinfo_write.h"
#include "relativeinfo_write.h"
#include "Relative.h"
#include "mainwindow.h"
#include "smtp.h"
#include "account.h"
Relative::Relative(QString name, QString phone, QString mail, QString birthday, QString sendBirth,
                     QString relation):Contact(name, phone, mail, birthday, sendBirth)
{
    this->relation = relation;
}

void Relative::saveInfo(QString username)
{
    //获取文件地址
    ...
    QString temp = "Relative," + Contact::getname() + "," + Contact::getphone() + "," + Contact::getmail() + "," + Contact::getbirth() + "," + Contact::getsendBirth() + "," + relation +  "\n";
    in<<temp;
    user_file.close();
}

QString Relative::getrelation()
{
    return relation;
}


relativeinfo_write::relativeinfo_write(QString username,QWidget *parent) :
    QWidget(parent),
    ui(new Ui::relativeinfo_write)
{
    this->username = username;
    ui->setupUi(this);
}

relativeinfo_write::~relativeinfo_write()
{
    delete ui;
}

void relativeinfo_write::on_okButton_clicked()
{
    ...
    Relative r = Relative(name, phone, mail, birthday, sendBirth, relation);
    if(name == "")
    {
...
void relativeinfo_write::on_cancelButton_clicked()
{
    MainWindow* NM = new MainWindow(username);
    NM->show();
    delete this;
