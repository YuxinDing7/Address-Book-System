...
#include <QDebug>
#include "account.h"
ColleagueModify::ColleagueModify(QString username, QString modifyName, QString returnWin, QString line_text,QWidget *parent) :
    QWidget(parent),
    ui(new Ui::ColleagueModify)
{
    ui->setupUi(this);
    ...
    QString company = contact_line.section(',', 6, 6);
    ui->company_lineEdit->setText(company);

    user_file.close();
}

ColleagueModify::~ColleagueModify()
{
    delete ui;
}

void ColleagueModify::on_okButton_clicked()
{
    ...
    Colleague o = Colleague(name, phone, mail, birthday, sendBirth2, company);
...
void ColleagueModify::on_cancelButton_clicked()
{
    if(returnWin == "MainWindow")
    {
        MainWindow* NM = new MainWindow(username);
