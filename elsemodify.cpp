...
#include "smtp.h"
#include "account.h"
ElseModify::ElseModify(QString username,QString modifyName, QString returnWin,QString line_text, QWidget *parent) :
    QWidget(parent),
    ui(new Ui::ElseModify)
{
    ui->setupUi(this);
    ...
    QString remark= contact_line.section(',', 6, 6);
    ui->remarkEdit->setText(remark);

    user_file.close();
}

ElseModify::~ElseModify()
{
    delete ui;
}

void ElseModify::on_okButton_clicked()
{
    ...
    Else e = Else(name, phone, mail, birthday, sendBirth2, remark);
    if(name == "")
    {
...
void ElseModify::on_cancelButton_clicked()
{
    if(returnWin == "MainWindow")
    {
        MainWindow* NM = new MainWindow(username);
