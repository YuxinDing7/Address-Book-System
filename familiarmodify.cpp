...
#include "smtp.h"
#include "account.h"
FamiliarModify::FamiliarModify(QString username, QString modifyName, QString returnWin,QString line_text, QWidget *parent) :
    QWidget(parent),
    ui(new Ui::FamiliarModify)
{
    ui->setupUi(this);
    ...
    QString job = contact_line.section(',', 9, 9);
    job = job.remove("\n");
    ui->job_lineEdit->setText(job);

    user_file.close();
}

FamiliarModify::~FamiliarModify()
{
    delete ui;
}

void FamiliarModify::on_okButton_clicked()
{
    ...
    Familiar f = Familiar(name, phone, mail, birthday, sendBirth2, school, grade, major, job);
    if(name == "")
    {
...
void FamiliarModify::on_cancelButton_clicked()
{
    if(returnWin == "MainWindow")
    {
        MainWindow* NM = new MainWindow(username);
